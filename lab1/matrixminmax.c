...
properties sums[MAXWORKERS]; /* partial sums */
...
void *Worker(void *arg) {
  ...
  sums[myid].sum = total;
  matrixcpy(&min, &sums[myid].min);
  matrixcpy(&max, &sums[myid].max);
  Barrier();
  if (myid == 0) {
    total = 0;
    for (i = 0; i < numWorkers; i++) {
      total += sums[i].sum;
      if (sums->min.val < min.val) {
        matrixcpy(&sums->min, &min);
      }
      if (sums->max.val < max.val) {
        matrixcpy(&sums->max, &max);
      }
    }
    /* get end time */
    end_time = read_timer();
